 ...
 function Messenger() {
  ...
  useEffect(() => {
    const getConversations = async () => {
      try {
        const res = await axios.get(AU + "conversations/" + user._id);
  ...
  useEffect(() => {
    const getMessages = async () => {
      try {
        const res = await axios.get(AU + "messages/" + currentChat?._id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(AU + "messages", message);
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      ...
      if (currentChat) {
        const x = await axios.get(AU + `users?userId=${currentChat.members[1]}`);
        setchathead(x.data);
