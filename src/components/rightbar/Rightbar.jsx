...
export default function Rightbar({ user }) {
  ...
  useEffect(() => {
    const getFriends = async () => {
      try {
        const friendList = await axios.get(AU + `users/friends/${user?user._id :currentUser._id }`);
          ...
          </div>
{
  ...
  const ProfileRightbar = () => {
    ...
    const handle = async () => {
      ...
      if (flag) {
        await axios.put(AU + `users/${user._id}/unfollow`, { "_id": currentUser._id })
        setflag(false)
        ...
        await axios.put(AU + `users/${user._id}/follow`, { "_id": currentUser._id })
        setflag(true)
